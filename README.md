...
## Distribution (single-file installers)

Hematite now includes packaging scripts to produce single distributable installer artifacts:
...
```bash
./packaging/build_release.sh linux-deb 0.1.0
```
...
```bash
./packaging/linux/package_deb.sh 0.1.0 amd64
```
...
```bash
./packaging/macos/package_dmg.sh 0.1.0
```
...
```powershell
pwsh ./packaging/windows/package_msi.ps1 -Version 0.1.0
```

Requires WiX Toolset (`candle.exe`, `light.exe`).
